...
#define __KALMAN_FILTERS_H_GLOBALS
#include "KalmanFilter.h"

void Kalman_init(void)
{
    Kalman1FilterCreate(&kalman_imu_pit,1,40);
    Kalman1FilterCreate(&kalman_yaw_angle_error, 1, 1);
    Kalman1FilterCreate(&kalman_yaw_aim_speed, 1, 5);
    Kalman1FilterCreate(&kalman_yaw_abs_speed, 1, 40);
    Kalman1FilterCreate(&kalman_bullet_time, 1, 30);

    Kalman1FilterCreate(&kalman_vision_dr, 1, 30);
    Kalman1FilterCreate(&kalman_vision_distance, 1, 30);
    
    Kalman1FilterCreate(&kalman_yaw_imu_speed, 1, 76);
    Kalman1FilterCreate(&kalman_predict_angle, 1, 30);
    Kalman1FilterCreate(&kalman_predict_distance, 1, 35);
    Kalman1FilterCreate(&kalman_2D_spd_arc, 1, 35);
    Kalman1FilterCreate(&kalman_object_angular_speed, 1, 30);
    
    Kalman1FilterCreate(&kalman_pit_energy, 1, 100);
    Kalman1FilterCreate(&kalman_yaw_energy, 1, 100);
...
void Kalman1FilterDeinit(Kalman1_param_t *p)
{
    ...
    p->P_last = 1; //上一时刻最优结果的协方差  P(k-1|k-1)
    p->kg = 0;     //kalman增益
}
