...
int judge_data_handler(uint8_t *ReadFromUsart)
{
    ...
    if(ReadFromUsart[ SOF ] == JUDGE_FRAME_HEADER)
    {
        ...
        if(*(ReadFromUsart + sizeof(FrameHeader) + LEN_CMDID + FrameHeader.data_length + LEN_TAIL) == 0xA5)
        {
            /* 如果一个数据包出现了多帧数据,则再次读取 */
            judge_data_handler(ReadFromUsart + sizeof(FrameHeader) + LEN_CMDID + FrameHeader.data_length + LEN_TAIL);
        }
    }
    return retval_tf;
}
