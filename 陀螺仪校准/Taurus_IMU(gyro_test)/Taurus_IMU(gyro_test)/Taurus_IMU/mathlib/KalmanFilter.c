...
Kalman1_param_t kalman_gyro_z;
Kalman1_param_t kalman_gyro_z_stop;
Kalman2_param_t kalman2_yaw_param = 
					{
				 		.P_data = {1, 0, 0, 1},
 				 		.A_data = {1, 0.005, 0, 1},//采样时间间隔
 				 		.H_data = {1, 0, 0, 1},
  				 	.Q_data = {1, 0, 0, 1},
 				 		.R_data = {20, 0, 0, 20}//200 400//500 1000				
					};//初始化部分kalman参数;
Kalman2_param_t kalman2_pitch_param = 
					{
				 		.P_data = {1, 0, 0, 1},
 				 		.A_data = {1, 0.005, 0, 1},//采样时间间隔
 				 		.H_data = {1, 0, 0, 1},
...
kalman2_filter_t kalman2_pitch_filter;					
...
//float32_t testOutput[TEST_LENGTH_SAMPLES]={0};               /* 滤波后的输出 */
float32_t testOutput[TEST_LENGTH_SAMPLES]={0};
...
const float32_t OrigalData1[512]={10.000};				
void Kalman_init(void)
{
  //CreateKalman1Filter(&kalman_gyro_z,1,30);
//	CreateKalman1Filter(&kalman_gyro_z,1,30);
	CreateKalman1Filter(&kalman_gyro_z_stop,0.1,700);
	CreateKalman1Filter(&kalman_gyro_z,0.2,10);
	CreateKalman2Filter(&kalman2_yaw_filter , &kalman2_yaw_param);	
	CreateKalman2Filter(&kalman2_pitch_filter , &kalman2_pitch_param);
}
/**
  * @brief 创建一阶卡尔曼滤波器
  * @param p：卡尔曼结构体；
...
float my_Kalman1Filter_calc(Kalman1_param_t* p,float dat,float R)
{
    /*-------------------预测-------------------*/   
		/*1.对下一时刻均值的预测：X̂k'=AX̂k-1+BUk	*/
	  p->R = R;
...
//  F->filtered_value[1] = F->xhat.pData[1];
  F->filtered_value[1] = F->xhatminus.pData[0];
//    F->filtered_value[0] = F->xhatminus.pData[0];
//		F->filtered_value[1] = F->xhatminus.pData[1];
...
//	//inputREF = (float32_t *)&OrigalData1[0];   	
...
//     
//	}
//	imu_output_data.Gyro.Z=outputF32[511];
//}
void kalman_R_calcu(void)
{
   arm_var_f32(gyro_z,50,&R);
	 arm_mean_f32(gyro_z,20,&average);
 //  CreateKalman1Filter(&kalman_gyro_z,0.1,R);
//	 result =(Kalman1Filter_calc(&kalman_gyro_z,imu_org_data.Gyro.Z)-Bias.Gyro.Z)/16.384/57.3f;
//	result = (my_Kalman1Filter_calc(&kalman_gyro_z,imu_org_data.Gyro.Z,R)-Bias.Gyro.Z)/16.384f/57.3f;
//	if(ABS(average)>=10)
//	{ result =(Kalman1Filter_calc(&kalman_gyro_z,imu_org_data.Gyro.Z)-Bias.Gyro.Z)/16.384f/57.3f;}
//	else 
//	{
//	  //result = (my_Kalman1Filter_calc(&kalman_gyro_z_stop,imu_org_data.Gyro.Z,R)-3.869f)/16.384f/57.3f;;
//		//result =(Kalman1Filter_calc(&kalman_gyro_z_stop,imu_org_data.Gyro.Z)-3.9698379f)/16.384f/57.3f;
//		  result = (imu_org_data.Gyro.Z-Bias.Gyro.Z)/16.384f/57.3f;
//	}
	//  result =(Kalman1Filter_calc(&kalman_gyro_z_stop,imu_org_data.Gyro.Z)-Bias.Gyro.Z)/16.384f/57.3f;
	 result = (imu_org_data.Gyro.Z -Bias.Gyro.Z)/16.384f;
//	 buf +=result*0.001*57.3f;
	 yaw_angle = buf+180;
	if(yaw_angle>=360)
	{
	    yaw_angle -=360;
...
//	 yaw_kf_result=Kalman2Filter_calc(&kalman2_yaw_filter,yaw_angle,JY901_org_data.Gyro_z);
//	 buf=yaw_kf_result[0];
}